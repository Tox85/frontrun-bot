    ...
    const pragmas = await Promise.all([
      new Promise<string>((resolve, reject) => {
        db.get("PRAGMA journal_mode", (err, row: any) => {
          if (err) reject(err);
          else resolve(row?.journal_mode || 'unknown');
        });
      }),
      new Promise<string>((resolve, reject) => {
        db.get("PRAGMA synchronous", (err, row: any) => {
          if (err) reject(err);
          else resolve(row?.synchronous || 'unknown');
        });
      }),
      new Promise<string>((resolve, reject) => {
        db.get("PRAGMA cache_size", (err, row: any) => {
          if (err) reject(err);
          else resolve(row?.cache_size || 'unknown');
        });
      })
    ]);
