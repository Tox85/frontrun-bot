...
class T0Validator {
  ...
  constructor(baseUrl: string, testBase: string, dryRun: boolean = false) {
   ...
   */
  async getMetricsRobust(): Promise<{
    new: number;
    dup: number;
    future: number;
    p95_detect_insert: number;
    p95_insert_order: number;
  }> {
    try {
   ...
   */
  async simulateNoticeLive(): Promise<void> {
    ...
    try {
      const response = await axios.post(`${this.baseUrl}/simulate/notice`, notice);
  ...
  async testDeduplication(): Promise<void> {
    ...
    for (let i = 0; i < 5; i++) {
      try {
        await axios.post(`${this.baseUrl}/simulate/notice`, notice);
   ...
   */
  async testFutureNotice(): Promise<void> {
   ...
   */
  validateResults(initial: any, final: any): {
    success: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
