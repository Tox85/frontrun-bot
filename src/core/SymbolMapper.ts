...
export class SymbolMapper {
  ...
  private exchangePatterns: Map<string, RegExp>;
   ...
   */
  private getFallbackQuote(originalQuote: string, targetExchange: string): string | null {
  ...
  normalizeBaseSymbol(base: string): string | null {
    ...
    if (!/^[A-Z0-9.]+$/.test(cleanBase)) return null;
  ...
  getStats(): {
    ...
    const supportedQuotes: Record<string, string[]> = {};
