...
class PositionSizer {
     ...
     */
    updateExchangeLimits(exchange, limits) {
     ...
     */
    calculatePositionSize(balance, exchange, symbol, currentPrice, customRiskPercentage) {
        ...
        try {
            ...
            const clampedNotional = Math.max(limits.minNotional, Math.min(limits.maxNotional, targetNotional));
            // Calculer le levier effectif
            const effectiveLeverage = Math.min(this.config.leverageTarget, limits.maxLeverage);
            ...
            // Validation finale
            const finalValidation = this.validatePosition(clampedNotional, positionSize, effectiveLeverage, limits);
            return {
     ...
     */
    validateInputs(balance, riskPct, limits) {
        const errors = [];
        if (balance <= 0) {
     ...
     */
    validatePosition(notional, positionSize, leverage, limits) {
        const errors = [];
     ...
     */
    calculatePositionSizeForAmount(targetAmount, exchange, currentPrice) {
        const upperExchange = exchange.toUpperCase();
        const limits = this.exchangeLimits.get(upperExchange);
        if (!limits) {
            return this.createErrorPosition(`Limites non trouvées pour ${exchange}`);
        ...
        try {
            ...
            if (targetAmount < limits.minNotional || targetAmount > limits.maxNotional) {
                return this.createErrorPosition(`Montant cible hors limites (${limits.minNotional} - ${limits.maxNotional})`);
            }
            // Calculer le levier optimal
            const optimalLeverage = Math.min(this.config.leverageTarget, limits.maxLeverage);
            // Calculer la taille de position
            const positionSize = targetAmount / optimalLeverage;
            // Validation
            const validation = this.validatePosition(targetAmount, positionSize, optimalLeverage, limits);
            return {
     ...
     */
    adjustPositionSize(originalSize, exchange) {
        const upperExchange = exchange.toUpperCase();
        const limits = this.exchangeLimits.get(upperExchange);
        if (!limits) {
            return this.createErrorPosition(`Limites non trouvées pour ${exchange}`);
        ...
        try {
            ...
            // Validation finale
            const validation = this.validatePosition(adjustedNotional, positionSize, adjustedLeverage, limits);
            return {
     ...
     */
    getSizingRecommendations(balance, exchange) {
        const conservative = this.calculatePositionSize(balance, exchange, 'BTCUSD', 50000, 0.01 // 1%
        );
        const moderate = this.calculatePositionSize(balance, exchange, 'BTCUSD', 50000, 0.02 // 2%
        );
        const aggressive = this.calculatePositionSize(balance, exchange, 'BTCUSD', 50000, 0.05 // 5%
        );
...
exports.positionSizer = PositionSizer.getInstance();
