...
exports.extractBaseFromNotice = extractBaseFromNotice;
...
function isValidBase(x) {
    if (!x)
        return false;
    if (x.length < 2 || x.length > 10)
        return false;
    if (QUOTE_BLACKLIST.has(x))
        return false;
    if (!/^[A-Z0-9.\-]+$/.test(x))
        return false;
    if (/[.\-]$/.test(x))
...
function extractBaseFromNotice(raw) {
    ...
    if (m && m[2]) {
        ...
        if (isValidBase(base))
            return { kind: 'LATIN', base, source: 'paren_ko_en', ctx: { ko: m[1] || '' } };
    }
    // 2) EN(KO) - TOWNS(타운즈)
    ...
    if (m && m[1]) {
        ...
        if (isValidBase(base))
            return { kind: 'LATIN', base, source: 'paren_en_ko' };
    }
    ...
    if (m && m[1]) {
        const base = m[1].toUpperCase();
        if (isValidBase(base))
            return { kind: 'LATIN', base, source: 'pair' };
    }
    ...
    if (m && m[1]) {
        const base = m[1].toUpperCase();
        if (isValidBase(base))
            return { kind: 'LATIN', base, source: 'krw_phrase' };
    }
...
//# sourceMappingURL=extractBase.js.map