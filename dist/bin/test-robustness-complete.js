...
async function waitForBot() {
    ...
    console.log('\n❌ Bot failed to start within 30 seconds');
    return false;
}
async function testRobustness() {
    console.log('🧪 Testing Complete Robustness Implementation\n');
    ...
    console.log('\n');
    try {
        // Test 1: /health avec nouvelles métriques
        ...
        const criticalChecks = [
            healthData.baseline_state && ['READY', 'CACHED', 'DEGRADED'].includes(healthData.baseline_state),
            healthData.baseline_cb_state && ['CLOSED', 'OPEN', 'HALF_OPEN'].includes(healthData.baseline_cb_state),
            typeof healthData.t0_enabled === 'boolean',
            healthData.leader_instance_id && healthData.leader_instance_id !== 'unknown'
        ];
        const passedChecks = criticalChecks.filter(Boolean).length;
        console.log(`\n🔍 Critical checks: ${passedChecks}/${criticalChecks.length} passed`);
        console.log('\n');
        // Test 2: /metrics avec circuit-breakers
        ...
        // Validation des métriques circuit-breaker
        if (metricsData.baseline) {
            console.log(`   ✅ Baseline circuit-breaker metrics available`);
        }
        if (metricsData.t0) {
            console.log(`   ✅ T0 circuit-breaker metrics available`);
        }
        console.log('\n');
        // Test 3: /status
...
//# sourceMappingURL=test-robustness-complete.js.map