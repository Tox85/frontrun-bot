...
async function testMigration() {
    ...
    const dbPath = path_1.default.join(__dirname, '../../data/test-migration.db');
    return new Promise((resolve, reject) => {
      ...
      `, (err) => {
        ...
        `, (err) => {
                    ...
                    db.all("PRAGMA table_info(perp_catalog)", (err, beforeColumns) => {
                        if (err) {
                            reject(err);
                            return;
                        }
            ...
            `, (err) => {
                            ...
                            db.all("PRAGMA table_info(perp_catalog)", (err, afterColumns) => {
                                if (err) {
                                    reject(err);
                                    return;
                                }
                                ...
                                db.all("SELECT * FROM perp_catalog", (err, data) => {
                                    if (err) {
                                        reject(err);
                                        return;
                                    }
                  ...
                  `, (err) => {
                                        ...
                                        db.get("SELECT * FROM perp_catalog WHERE exchange = 'BINANCE'", (err, newData) => {
                                            if (err) {
                                                reject(err);
                                                return;
                                            }
