...
exports.testUpsertRobustness = testUpsertRobustness;
...
async function testUpsertRobustness() {
    ...
    const perpCatalog = new PerpCatalog_1.PerpCatalog(db, 10000); // 10s
    try {
        ...
        const count1 = await new Promise((resolve, reject) => {
            db.get('SELECT COUNT(*) as count FROM perp_catalog WHERE exchange = ? AND base = ?', ['TEST_EXCHANGE', 'BTC'], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row.count);
        ...
        const updatedRow = await new Promise((resolve, reject) => {
            db.get('SELECT * FROM perp_catalog WHERE exchange = ? AND base = ?', ['TEST_EXCHANGE', 'BTC'], (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row);
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                db.run('INSERT INTO perp_catalog (exchange, base, quote, symbol, leverage_max) VALUES (?, ?, ?, ?, ?)', ['TEST_EXCHANGE', 'BTC', 'USDT', 'BTCUSDT_DUPLICATE', 100], (err) => {
                    if (err) {
        ...
        const finalCount = await new Promise((resolve, reject) => {
            db.get('SELECT COUNT(*) as count FROM perp_catalog', (err, row) => {
                if (err)
                    reject(err);
                else
                    resolve(row.count);
...
//# sourceMappingURL=test-upsert-robustness.js.map