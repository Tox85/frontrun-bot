...
class PreflightChecker {
    results = [];
    logger = new StructuredLogger_1.StructuredLogger(StructuredLogger_1.LogLevel.INFO);
    async run() {
    ...
    async checkEnvConfig() {
        ...
        const configValidation = (0, env_1.validateConfig)();
        // Vérifications spécifiques selon les règles
        const errors = [];
        const warnings = [];
        // TELEGRAM_BOT_TOKEN (format)
        if (env_1.CONFIG.TELEGRAM_ENABLED && !env_1.CONFIG.TELEGRAM_BOT_TOKEN) {
            errors.push('TELEGRAM_BOT_TOKEN manquant');
        }
        else if (env_1.CONFIG.TELEGRAM_ENABLED && env_1.CONFIG.TELEGRAM_BOT_TOKEN && !env_1.CONFIG.TELEGRAM_BOT_TOKEN.startsWith('5')) {
            warnings.push('TELEGRAM_BOT_TOKEN format suspect (doit commencer par "5")');
        }
        // TELEGRAM_CHAT_ID (numérique)
        if (env_1.CONFIG.TELEGRAM_ENABLED && !env_1.CONFIG.TELEGRAM_CHAT_ID) {
            errors.push('TELEGRAM_CHAT_ID manquant');
        }
        else if (env_1.CONFIG.TELEGRAM_ENABLED && env_1.CONFIG.TELEGRAM_CHAT_ID && isNaN(Number(env_1.CONFIG.TELEGRAM_CHAT_ID))) {
            errors.push('TELEGRAM_CHAT_ID doit être numérique');
        }
        // TRADING_ENABLED (bool)
        if (env_1.CONFIG.HL_ENABLED && !env_1.CONFIG.HL_WALLET) {
            errors.push('TRADING_ENABLED=true mais HYPERLIQUID_WALLET_ADDRESS manquant');
        }
        // MAX_POSITION_SIZE_USD
        if (env_1.CONFIG.RISK_PER_TRADE_USD <= 0) {
            errors.push('RISK_PER_TRADE_USD doit être > 0');
        }
        // RISK_PERCENT
        if (env_1.CONFIG.RISK_PCT <= 0 || env_1.CONFIG.RISK_PCT > 1) {
            errors.push('RISK_PCT doit être entre 0 et 1');
        }
        // T0_POLL_INTERVAL_MS >= 1100
        if (env_1.CONFIG.UPBIT_POLL_MS < 1100) {
            errors.push('UPBIT_POLL_MS doit être >= 1100ms (actuel: ' + env_1.CONFIG.UPBIT_POLL_MS + 'ms)');
        }
        // T0_HTTP_TIMEOUT_MS
        if (env_1.CONFIG.UPBIT_TIMEOUT_MS <= 0) {
            errors.push('UPBIT_TIMEOUT_MS doit être > 0');
        }
        // T0_HTTP_RETRIES
        if (env_1.CONFIG.UPBIT_MAX_RETRIES < 0) {
    ...
    async checkDatabase() {
        ...
        try {
            ...
            const migrationsResult = await new Promise((resolve, reject) => {
    ...
    async checkBaseline() {
        ...
        try {
            ...
            const krwCount = await new Promise((resolve, reject) => {
    ...
    async checkHttpServer() {
        ...
        try {
            ...
            const testPort = env_1.CONFIG.PORT;
    ...
    async checkHyperliquid() {
        console.log('💎 Vérification Hyperliquid...');
        if (!env_1.CONFIG.HL_ENABLED) {
            this.results.push({
                section: 'Hyperliquid',
                status: '⚠️',
        ...
        try {
            ...
            const httpClient = new HttpClient_1.HttpClient('preflight-hl', {
                timeoutMs: 5000,
                maxRetries: 1,
            ...
            const testnetUrl = env_1.CONFIG.HL_TESTNET ? 'https://api.hyperliquid-testnet.xyz' : 'https://api.hyperliquid.xyz';
            try {
                ...
                if (response.status === 200) {
                    this.results.push({
                        section: 'Hyperliquid',
                        status: '✅',
                        message: `API ${env_1.CONFIG.HL_TESTNET ? 'testnet' : 'mainnet'} accessible`,
                        critical: false
                    });
                }
                else {
    ...
    async checkTelegram() {
        ...
        if (!env_1.CONFIG.TELEGRAM_ENABLED) {
            this.results.push({
                section: 'Telegram',
                status: '⚠️',
        ...
        try {
            // Vérifier les tokens
            if (!env_1.CONFIG.TELEGRAM_BOT_TOKEN || !env_1.CONFIG.TELEGRAM_CHAT_ID) {
                this.results.push({
                    section: 'Telegram',
                    status: '❌',
