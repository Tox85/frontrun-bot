...
async function main() {
    ...
    try {
        ...
        db = new sqlite3_1.Database(BOT_CONFIG.DATABASE_PATH);
        ...
        singletonGuard = new SingletonGuard_1.SingletonGuard(db);
        ...
        if (!isLeader) {
            ...
            httpServer = new HttpServer_1.HttpServer(db, null, // baselineManager
            null, // perpCatalog
        ...
        if (!hyperliquid) {
            ...
            noticeClient = new NoticeClient_1.NoticeClient('https://api.bithumb.com/v1/notices', new HttpClient_1.HttpClient('NoticeClient', {
                timeoutMs: 5000,
                maxRetries: 3,
                baseRetryDelayMs: 250,
                maxRetryDelayMs: 500,
                jitterPercent: 20
            }), watermarkStore, BOT_CONFIG.LOG_DEDUP_WINDOW_MS, BOT_CONFIG.LOG_DEDUP_MAX_PER_WINDOW);
            ...
            if (baselineManager.canActivateT0()) {
                ...
                t0Interval = setInterval(async () => {
                    try {
                        ...
                        const latencyMetrics = Latency_1.latency.getMetrics();
            ...
            httpServer = new HttpServer_1.HttpServer(db, baselineManager, perpCatalog, singletonGuard, noticeClient, wsWatcher, telegramService, null, // tradeExecutor
        ...
        console.log('🌐 Starting HTTP server...');
        httpServer = new HttpServer_1.HttpServer(db, baselineManager, perpCatalog, singletonGuard, noticeClient, wsWatcher, telegramService, tradeExecutor, healthMonitor, eventStore, // eventStore
        {
            port: BOT_CONFIG.HTTP_PORT,
            host: '0.0.0.0',
            enableCors: true,
